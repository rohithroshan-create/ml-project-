...
if GEMINI_API_KEY:
    try:
        ...
        gemini_model = genai.GenerativeModel("gemini-2.0-flash-lite") # <-- You may need to update this based on API support
            ...
            if st.button("Predict Delay"):
                if catboost_delay_reg:
                    try:
                        ...
                        st.write(f"**Mean:** {np.mean(pred):.2f}, **Std:** {np.std(pred):.2f}, **Min:** {np.min(pred):.2f}, **Max:** {np.max(pred):.2f}")
            ...
            if st.button("Predict Risk"):
                if catboost_delivery_risk:
                    try:
                        ...
                        st.dataframe(pred_df)
                        st.write(f"**Risk Counts:** {pd.Series(pred).value_counts().to_dict()}")
